...
*/
LPTEXTURE CGame::LoadTexture(LPCSTR texturePath)
{
	...
	HRESULT hr = D3DX10GetImageInfoFromFileA(texturePath, NULL, &imageInfo, NULL);
	if (FAILED(hr))
	{
		DebugOut((wchar_t*)"[ERROR] D3DX10GetImageInfoFromFile failed for  file: %s with error: %d\n", texturePath, hr);
		return NULL;
	}
